...
describe("DELETE /api/admin/product/:variantId", () => {
  ...
  test("Delete variant with correct request body and role", async () => {
    const response = await request(app)
      .delete(`/api/admin/product/${productVariant._id}`)
      .set("Cookie", `swf_access=${accessToken}`);

    expect(response.status).toBe(200);
    expect(response.body.message).toBe("Delete operation successful");
    expect(response.body.code).toBe("VARIANT_DELETED");
  });
});
